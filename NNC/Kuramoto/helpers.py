...
class HNC_strategy(nn.Module):
    def __init__(self, skip, dim, layers,batch, device):
        ...
        h_layers = (1+dim,)*layers+(8,)
            
        self.polyf = MLP(in_size=2*dim, out_size=dim,mlp_size=dim,num_layers=1)        
...
class tHNC_strategy(nn.Module):
    def __init__(self, skip, dim, layers,batch, device):
        ...
        self.polyf = MLP(in_size=3*dim, out_size=dim,mlp_size=2*dim,num_layers=1)        
...
class ENC_strategy(nn.Module):
    def __init__(self, skip, dim, layers,batch, device, emb_layers=0):
        ...
        self.poly = MLP(in_size=1+3*dim, out_size=dim,mlp_size=(1+2*dim),num_layers=1+layers)   
        
        if emb_layers==0:
            self.embed=torch.nn.Identity()
        else:
            self.embed = MLP(in_size=1+dim, out_size=1+dim, mlp_size=1+dim, num_layers=emb_layers)
        self.device=device
