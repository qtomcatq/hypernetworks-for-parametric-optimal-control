...
class ENC_strategy(nn.Module):
    def __init__(self, dim,outputdim, layers,batch,device, skip, amat=None):
        ...
        self.device = device
        if skip:
            self.polyf = StableResNODEMLP(in_size=1+2*self.dim,out_size=outputdim,mlp_size=1+2*self.dim,num_layers=1+layers)  
        else:
            self.polyf = MLP(in_size=1+2*self.dim,out_size=outputdim,mlp_size=1+2*self.dim,num_layers=1+layers)  
